...
export class Proyek {
    editMode = 'list';
    accessList = null;
    table = document.querySelector('#proyek-table');
    tablePembayaran = document.querySelector('#pembayaran-proyek-table');
    tableSKK = document.querySelector('#skk-proyek-table');
    tableOperasionalProyek = document.querySelector('#operasional-proyek-table');
    tablePengajuanSKK = document.querySelector('#pengajuan-skk-proyek-table');
    dataTable = null;
    dataTablePembayaran = null;
    dataTableSKK = null;
    dataTableOperasionalProyek = null;
    dataTablePengajuanSKK = null;
    ...
    constructor() {
        console.log('%c Proyek constructor...', 'color: blue');
    }
    
    list(token) {
        const serverSideSetup = {
            ajax: {
                url: `${SITE_URL}proyek/get/datatable`,
                type: 'POST',
                headers: { 
        ...
        this.dataTable.createdRow = (row, data) => {
            ...
            const projectStatus = $('td:eq(8)', row).text();
            const badge = projectStatus == 'Berjalan' ? 'badge-info' : 'badge-success';
            $('td:eq(8)', row).addClass('text-center').html(`<span class="badge ${badge}">${projectStatus}</span>`);

            const actionTd = $('td:eq(11)', row)[0];
    ...
    new() {
        console.log('%c New Clicked...', 'color: blue');

        window.location.href = `${SITE_URL}proyek/form`;
    }
